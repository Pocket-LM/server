...
async def run_agent_message(graph, config: RunnableConfig, query: str) -> None:
    ...
    result = await graph.ainvoke(
        {"messages": [human], "history": [human]}, config, stream_mode="values"
    )
    print(result)
    ...
    # ):
    #     """
    #     This is for the model with reasoning capability, like "gemini-2.5-flash" which returns dict messages
